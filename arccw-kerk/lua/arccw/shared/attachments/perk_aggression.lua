att.PrintName = "Aggression"
att.Icon = Material("entities/acwatt_perk_streetjustice.png", "smooth mips")
att.Description = "Concentrate all your fury into every bash for increased melee damage. Your fury allows for faster reloading. The downside is bad recoil control."
att.Desc_Pros = {
}
att.Desc_Cons = {
}
att.Slot = "perk"

att.NotForNPC = true

att.AutoStats = true
att.Mult_MeleeDamage = 1.50
att.Mult_MeleeTime = 1.35
att.Mult_DrawTime = 0.9
att.Mult_ReloadTime = 0.6

att.Mult_Recoil = 1.35
att.Mult_RecoilSide = 1.35
att.Mult_VisualRecoilMult = 1.35
att.Mult_SightTime = 1.20
att.Mult_CycleTime = 1.25